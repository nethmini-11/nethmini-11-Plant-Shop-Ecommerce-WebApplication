...
import com.ecom.store.store.repository.CartItemRepository;
...
public class ShoppingCartServiceImpl implements ShoppingCartService {
	...
	public CartItem addPlantToShoppingCart(Plant plant, User user, int qty, String size) {
		ShoppingCart shoppingCart = this.getShoppingCart(user);
		CartItem cartItem = shoppingCart.findCartItemByPlantAndSize(plant.getId(), size);
	...
	public void updateCartItem(CartItem cartItem, Integer qty) {
		...
		} else if (cartItem.getPlant().hasStock(qty)) {
			cartItem.setQty(qty);
